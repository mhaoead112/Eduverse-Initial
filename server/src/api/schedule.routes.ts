 ...
 * Get schedule events for a specific date (for student dashboard)
 */
router.get('/events', isAuthenticated, async (req, res) => {
  try {
    ...
    if (userRole === 'student') {
      ...
      if (courseIds.length > 0) {
        // Get events for enrolled courses on this date
        const courseEvents = await db
          .select({
            id: events.id,
            title: events.title,
        ...
        const assignmentsDue = await db
          ...
          .where(
            and(
              ...
              lte(assignments.dueDate, endOfDay)
            )
          );
        ...
        scheduleEvents = [...courseEvents.map(e => ({ ...e, type: e.eventType })), ...assignmentEvents];
      }
    } else {
      // For teachers/admins, get all events on this date
      const allEvents = await db
        .select({
          id: events.id,
          title: events.title,
      ...
      scheduleEvents = allEvents.map(e => ({ ...e, type: e.eventType }));
    }
